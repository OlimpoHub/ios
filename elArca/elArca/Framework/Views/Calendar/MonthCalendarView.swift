...
struct MonthCalendarView: View {
    ...
    var body: some View {
        ScrollView(.horizontal) {
            LazyHStack(spacing: .zero) {
                ForEach(months) { month in
                    VStack {
                        MonthView(month: month, dragProgress: dragProgess, focused: $focused, selectedDate: $selection)
                            .offset(y: (1 - dragProgess) * verticalOffset(for: month))
        ...
        .onChange(of: dragProgess) { _, newValue in
